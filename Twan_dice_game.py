...
#   Strategy simulator for the Twan dice game
# - two strategies are applied to players 1 and 2, a conservative and gambling profiles
# - code executes n random games with these two players and keeps the final score

# - players take turns starting with six dice, 1s and 5s are 100 and 50 points respectively. The rest are
#   reused in another round until a throw fails to bring points, that is a bust. A player can decide to
#   stop and keep the accumulated points and pass the play to the next player who can decide to 'gamble'
#   and continue playing with the round's points and dice or ask for a new set of six dice and 0 points.
#   If a six dice throw returns all different scores 1-6 that scores 1500 points and the player continues
#   with a fresh set of six. Also, if a player 'spends' all dice by scoring with 1s and/or 5s then again
#   a new set of six dice is awarded.

# - *spoiler alert*         <<<the conservative player wins more often>>>
